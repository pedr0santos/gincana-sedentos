...
const SESSION_APP_ID = ENV.appId || "gincana-sedentos";
...
class SDKServer {
  ...
  async verifySession(
    ...
    try {
      ...
      return {
        ...
        appId,
        name,
        ...(typeof sessionVersion === "number" ? { sessionVersion } : {}),
      };
    } catch (error) {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    // Preview-only fallback. Production accepts only the HttpOnly cookie so
    // session tokens never need to be exposed to application JavaScript.
